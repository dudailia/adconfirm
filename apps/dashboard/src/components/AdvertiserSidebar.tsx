...
const navLinks = [
  ...
  { href: "/advertiser/campaigns/new", label: "New Campaign", Icon: PlusCircle },
];

export function AdvertiserSidebar({ advertiserName }: { advertiserName: string }) {
  ...
  return (
    <aside className="w-56 shrink-0 flex flex-col bg-surface border-r border-border min-h-screen">
      <div className="px-4 py-5 border-b border-border">
        ...
        <div className="text-xs text-muted-fg mt-2 truncate">{advertiserName}</div>
      </div>

      <nav className="flex-1 px-3 py-4 space-y-1">
        {navLinks.map(({ href, label, Icon }) => {
          const active = pathname === href ||
            (href !== "/advertiser/dashboard" && pathname.startsWith(href));
          return (
            <Link
              key={href}
              href={href}
