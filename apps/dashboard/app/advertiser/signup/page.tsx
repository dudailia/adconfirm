...
export default function AdvertiserSignupPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    window.location.replace("/advertiser/dashboard");
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-md rounded-xl border border-border bg-surface p-8 shadow-xl">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-fg">Create your advertiser account</p>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="website" className="mb-1 block text-sm font-medium text-muted-fg">
              Website URL
            </label>
            <input
              id="website"
        ...
        <p className="mt-6 text-center text-xs text-muted-fg">
          ...
          <Link href="/advertiser/login" className="text-accent hover:underline">
            Sign in
          </Link>
        </p>
      </div>
